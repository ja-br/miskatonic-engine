...
/**
 * Detect if device is touch-only (mobile/tablet without mouse)
 * Uses pointer media queries instead of UA sniffing
 */
function isMobileDevice(): boolean {
  // Check if the primary pointer is coarse (touch) and there's no fine pointer (mouse)
...
/**
 * Initialize mobile warning if on touch-only device
 */
function initMobileWarning(): void {
  if (isMobileDevice()) {
...
async function main(): Promise<void> {
  // Initialize mobile warning (synchronous, runs immediately)
  initMobileWarning();
