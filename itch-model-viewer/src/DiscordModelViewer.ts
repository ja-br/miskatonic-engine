...
export class DiscordModelViewer {
  ...
  async loadModel(modelName: string): Promise<void> {
    ...
    try {
      if (modelName === 'sphere') {
        ...
        // Accept either full paths (with directory) or short names
        let path: string;
        ...
        if (modelName.endsWith('.obj')) {
          // If it's a .obj file, it must include a path (has slashes)
          if (modelName.includes('/')) {
            path = modelName;
          } else {
            // Just a filename without path is invalid
            console.error(`Model name must include path, got: "${modelName}". Using default model.`);
            path = modelPaths['naked-snake'];
          }
        } else {
          // Use legacy short name lookup
          path = modelPaths[modelName] || modelPaths['naked-snake'];
          if (!modelPaths[modelName]) {
            console.warn(`Unknown model short name: "${modelName}". Using default model.`);
          }
        }
  ...
  private getBasePath(modelName: string): string {
    ...
    // If it's a full path (ends with .obj and contains /), extract the directory
    if (modelName.endsWith('.obj') && modelName.includes('/')) {
